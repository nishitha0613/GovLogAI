...
export const RawLogTerminal: React.FC<RawLogTerminalProps> = ({ customLogs, customRawText }) => {
  const { logs: defaultLogs } = useApp();
  const logsToDisplay = customLogs || defaultLogs;
    ...
    <Card className="bg-[#05080f] border border-slate-800 p-0 overflow-hidden font-mono text-xs shadow-2xl">
      ...
      <div className="px-4 py-2.5 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1 px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] transition cursor-pointer"
            title="Copy Raw Text"
          >
